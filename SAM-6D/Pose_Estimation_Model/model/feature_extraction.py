...
class ViT_AE(nn.Module):
    def __init__(
        ...
        if self.up_type == "linear":
            self.output_upscaling = nn.Linear(
                self.embed_dim * nblock, 16 * self.out_dim, bias=True
            )
        elif self.up_type == "deconv":
            self.output_upscaling = nn.Sequential(
                nn.ConvTranspose2d(
                    self.embed_dim * nblock, self.out_dim * 2, kernel_size=2, stride=2
                ),
                LayerNorm2d(self.out_dim * 2),
                nn.GELU(),
                nn.ConvTranspose2d(
                    self.out_dim * 2, self.out_dim, kernel_size=2, stride=2
                ),
            )
        else:
            assert False
        ...
        if self.pretrained:
            vit_checkpoint = os.path.join(
                "checkpoints", "mae_pretrain_" + self.vit_type + ".pth"
            )
            if not os.path.isdir(vit_checkpoint):
                if not os.path.isdir("checkpoints"):
                    os.makedirs("checkpoints")
                model_zoo.load_url(
                    "https://dl.fbaipublicfiles.com/mae/pretrain/mae_pretrain_"
                    + self.vit_type
                    + ".pth",
                    "checkpoints",
                )
    ...
    def forward(self, x):
        ...
        if self.up_type == "linear":
            x = (
                ...
                .reshape(B, 14, 14, 4, 4, self.out_dim)
                .permute(0, 5, 1, 3, 2, 4)
                .contiguous()
            )
...
class ViTEncoder(nn.Module):
    ...
    def forward(self, end_points):
        ...
        if (
            not self.training
            and "dense_po" in end_points.keys()
            and "dense_fo" in end_points.keys()
        ):
            ...
            dense_po, dense_fo = self.get_obj_feats(
                [tem1_rgb, tem2_rgb], [tem1_pts, tem2_pts], [tem1_choose, tem2_choose]
            )
