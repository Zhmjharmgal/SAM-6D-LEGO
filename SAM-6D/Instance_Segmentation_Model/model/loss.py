...
class MaskedPatch_MatrixSimilarity(nn.Module):
    ...
    def compute_straight(self, query, reference):
        ...
        sim_matrix = torch.matmul(
            query, reference.permute(0, 2, 1)
        )  # N_query x N_query_mask x N_refer_mask
        ...
        scores = (
            torch.sum(max_ref_patch_score, dim=-1) / factor
        )  # N_query x N_objects x N_templates
    ...
    def compute_similarity(self, query, reference):
        ...
        for idx_obj in range(N_objects):
            sim_matrix = torch.matmul(
                queries, references[:, idx_obj].permute(0, 1, 3, 2)
            )  # N_query x N_templates x N_query_mask x N_refer_mask
            similarity.append(sim_matrix)
        similarity.stack()
        similarity = similarity.data
        similarity = similarity.permute(
            1, 0, 2, 3, 4
        )  # N_query x N_objects x N_templates x N1_query x N2_ref
